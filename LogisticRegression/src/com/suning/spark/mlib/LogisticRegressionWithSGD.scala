...
object LogisticRegressionWithSGD {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val path:String = "/user/spark/model/LogisticRegressionWithSGD"
    val logger = Logger.getLogger(LogisticRegressionWithSGD.getClass);
    val sparkConf = new SparkConf().setAppName("LogisticRegressionWithSGD")
 ...
 //  val algorithm = new LassoWithSGD()
  ...
  /*  val algorithm=  new LogisticRegressionWithLBFGS()
    ...
    algorithm.optimizer.setRegParam(0.01119966)
...
*/
    ...
    val areaUnderPR_areaUnderROC= sc.makeRDD(List("Test areaUnderPR = ${metrics.areaUnderPR()}.","Test areaUnderROC = ${metrics.areaUnderROC()}."))
    ...
    val rmse = math.sqrt(loss / test.count())
     ...
     logger.info("----------------------------------------------------------------------")
     sc.makeRDD(List(rmse)).saveAsTextFile(path+"/rmse1.txt")
     sc.stop()
